...
def upload_to_qdrant(
    collection_name: str,
    embeddings_and_metadata: List[Dict[str, Any]],
    vector_size: int,
) -> None:
    """
    ...
    operation_info = qdrant_client.upsert(
        ...
        points=[
            PointStruct(
                id=file["id"], vector=file["vector"], payload={"text": file["text"]}
            )
...
def search_answer_in_qdrant(
    collection_name: str, query_embedding: List[float], db_chunks_number: int
) -> List[Dict[str, Any]]:
    """
    ...
    for point in search_result.points:
        answers.append(
            {"id": point.id, "score": point.score, "text": point.payload["text"]}
        )
